    ...
    return DropdownButton<String>(
      ...
      onChanged: (val) async {
        ...
        try {
          ...
          final previousRaw =
              (u['type'] as String?) ?? UserRole.customer.dbName;
          final previous = previousRaw.toLowerCase();

          debugPrint(
            'Admin changing role for userId=$userId from $previous to ${role.dbName}',
          );
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Role de ${u['name'] ?? ''} alterada para ${role.dbName}',
              ),
            ),
          );
        } catch (e, st) {
